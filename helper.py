...
# write_path_brc = '/global/scratch/jsimcock/data_files/covars/ssp_gdppc/{ssp}/{model}/{year}/{version}.nc'
...
if __name__ == '__main__':
  ...
  for ssp,model in itertools.product(ssps, models):
    ...
    for y in range(2010, 2100):
      window = range(y-12, y+1)
      #print(window)
      paths = [covar_path.format(ssp=ssp, model=model,year=yr) for yr in window]
      #print(paths)
      mdata['dependencies'] = str(paths)
      mdata['year'] = y 

      gen_kernel_covars(paths, kernel = 13, metadata=mdata, write_path=write_path)
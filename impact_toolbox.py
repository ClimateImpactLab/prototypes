...
def prep_gammas(path, p=4, seed=None):
    '''
...
def compute_betas(clim_covar, gdp_covar, gammas):
    '''
...
def precompute_baseline(weather_model_paths, gdp_covar_path, climate_covar_path, gammas, metadata, begin, end, poly=None, write_path=None):
    ...
    for p in range(2, poly + 1):
        annual_weather_paths_poly_tas = weather_model_paths.format(ssp=metadata['ssp'], model=metadata['model'], poly=p)
